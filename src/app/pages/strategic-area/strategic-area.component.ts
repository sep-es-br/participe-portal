...
export class StrategicAreaComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.locality = participationState ? participationState.locality : undefined;
    await this.loadStrategicArea();
    window.scrollTo(0, 0);
  }

  async loadStrategicArea() {
    const { success, data } = await this.participationSrv.getPlanItem(
      this.conferenceSrv.ConferenceActiveId, this.locality ? this.locality.id as number : null, null, ''
    );
    if (success) {
      this.strategicArea = data;
      if (!(this.locality && this.locality.id)) {
        this.breadcrumbSrv.removeLast();
      }
  ...
  async search() {
    ...
    .getPlanItem(this.conferenceSrv.ConferenceActiveId, this.locality ? this.locality.id as number : null, null, this.textSearch);
    if (success) {
